pipeline {
    ...
    environment {
        imageName = "fleetman-position-simulator"
        registry = ''
    ...
    stages {
        ...
        stage('Trigger K8S Manifest Updating') {
            steps {
                build job: 'k8s-update-manifests-fleetman-position-simulator', parameters: [string(name: 'DOCKERTAG', value: commit_id), string(name: 'BRANCH_GIT', value: branch_git)]
            }
        }
    }
}
